...
import {
  airlinesFromOpenFlightsDat,
  airlinesToJsonBlob,
} from '#/lib/airlines-csv'
import { airlinesList, ensureAirlinesLoaded } from '#/lib/airlines-client'

export const Route = createFileRoute('/_main/admin/airlines')({
  component: AirlinesAdminPage,
})

function AirlinesAdminPage() {
  const [imported, setImported] = useState<Airline[] | null>(null)
  const [bundled, setBundled] = useState<Airline[] | null>(null)
  ...
  const loadBundled = useCallback(async () => {
    setBundleErr(null)
    try {
      await ensureAirlinesLoaded()
      setBundled([...airlinesList])
    } catch (e) {
  ...
  const onFile = useCallback((file: File | null) => {
    ...
    reader.onload = () => {
      try {
        ...
        const rows = airlinesFromOpenFlightsDat(text)
        setImported(rows)
      } catch (e) {
  ...
  const headerNote = imported
    ? `Preview from ${fileName ?? 'airlines.dat'} (${shown.length} airlines)`
    : `Airlines in public bundle (${shown.length})`

  const downloadHref = useMemo(() => {
    if (!imported?.length) return null
    return URL.createObjectURL(airlinesToJsonBlob(imported))
  }, [imported])
    ...
    <main className="page-wrap px-4 py-8">
      <section className="island-shell rounded-2xl p-6 sm:p-8">
        ...
        <p className="m-0 mb-4 max-w-2xl text-sm leading-6 text-[var(--sea-ink-soft)]">
          ...
          (CSV). Optional header row{' '}
          <code className="text-xs">AirlineID,Name,Alias,IATA,ICAO,Callsign,Country,Active</code> is
          detected. Output is <code className="text-xs">Active=Y</code> rows with a valid 2-letter
          IATA and a resolvable ISO country code. Commit the JSON as{' '}
          <code className="rounded bg-[var(--chip-bg)] px-1.5 py-0.5 text-xs">
            public/data/airlines.json
          </code>{' '}
          (served at <code className="text-xs">/data/airlines.json</code>). Cached in IndexedDB after
          the first successful fetch.
        </p>

        <div className="mb-4 flex flex-wrap items-center gap-3">
          <label className="inline-flex cursor-pointer flex-col gap-1 text-sm text-[var(--sea-ink)]">
            <span>Import airlines.dat</span>
            <input
              type="file"
              accept=".dat,.csv,text/csv,text/plain"
        ...
        <div className="max-h-[min(28rem,55vh)] overflow-auto rounded-lg border border-[var(--line)]">
            ...
            <tbody>
              {shown.map((a) => (
                ...
                >
                  ...
                  <td className="px-2 py-1.5 text-[var(--sea-ink-soft)]">{a.name}</td>
          ...
          {shown.length === 0 ? (
            <p className="m-0 px-3 py-6 text-center text-sm text-[var(--sea-ink-soft)]">
              No airlines yet. Import <code className="text-xs">airlines.dat</code> or add{' '}
              <code className="text-xs">public/data/airlines.json</code>.
            </p>
          ) : null}
        </div>
      </section>
